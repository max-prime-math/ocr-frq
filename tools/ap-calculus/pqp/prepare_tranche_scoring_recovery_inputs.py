...
def main() -> int:
    ...
    for row in catalog["questions"]:
        ...
        output = args.output_dir / f"{row['id']}-scoring-guide.pdf"
        state = "not-written"
        if args.write:
    ...
    if args.write:
        (args.output_dir / "submission-queue.json").write_text(json.dumps({"schemaVersion": 1, "submission": "not-submitted", "reason": "legacy scoring-guide segmentation is untrusted", "artifacts": queue}, indent=2) + "\n", encoding="utf-8")
